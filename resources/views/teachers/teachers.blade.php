...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0">{{ trans('teachers.page_title') }}</h4>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                <li class="breadcrumb-item"><a href="#" class="default-color">{{ trans('dashboard.home') }}</a></li>
                <li class="breadcrumb-item active">{{ trans('teachers.page_topic') }}</li>
            </ol>
        </div>
    </div>
</div>
    ...
    <div class="card card-statistics h-100">
        <div class="card-body">
            <a href="{{route('teachers.create')}}" class="btn btn-success btn-sm" role="button"
               aria-pressed="true">{{ trans('teachers.new_teacher') }}</a><br><br>
            <div class="table-responsive">
                <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                    ...
                    <tr>
                        ...
                        <th>{{trans('teachers.teacher_specialization')}}</th>
                        <th>{{trans('teachers.teacher_gender')}}</th>
                        <th>{{trans('teachers.teacher_dateofhiring')}}</th>
                    ...
                    @foreach ($teachers as $teacher )
                        ...
                        <td>{{$teacher->joining_date}}</td>
                            <td>
                                ...
                                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_Teacher{{$teacher->id}}" title="{{ trans('teachers.delete_title') }}"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                                ...
                                <form action="{{route('teachers.destroy','id')}}" method="post">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 style="font-family: 'Cairo', sans-serif;" class="modal-title" id="exampleModalLabel">{{trans('teachers.delete_title')}}</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            <p> {{ trans('teachers.delete_submit') }}</p>
